...
if __name__ == '__main__':
    ...
    rospy.init_node('turtlebot3_dqn')  # 初始化ros节点，并命名为turtlebot3_dqn
    env = Env()  # 创建初始环境
    ...
    for episode in range(Max_episode):
        ...
        while True:  # 开始一个episode，该episode结束的条件：机器人到达终点 或 机器人撞上障碍物
            ...
            a = (model.select_action(np.array(s)) + np.random.normal(0, max_action * expl_noise, size=action_dim)
                 ).clip(-max_action, max_action)  # action
        ...
        if episode == 0:
            all_ep_r.append(episode_reward_sum)
        else:
            all_ep_r.append(all_ep_r[-1] * 0.9 + episode_reward_sum * 0.1)  # 为了更平滑
        writer.add_scalar('s_ep_r', all_ep_r[-1], global_step=episode)
        writer.add_scalar('ep_r', episode_reward_sum, global_step=episode)
        writer.add_scalar('explore', expl_noise, global_step=episode)
        print('episode:', episode, 'score:', episode_reward_sum, 'step:', steps)
